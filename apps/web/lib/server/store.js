...
export const storeAdapter = {
  ...
  async createCustomer(context, payload) {
    ...
    const newCustomer = {
      ...
      name: custName,
      company: custName,
      industry: payload.industry || payload.domain || "General",
      contactName: payload.contactName || payload.contact || "Primary Contact",
      email: payload.email || (payload.primaryContact && payload.primaryContact.includes("@") ? payload.primaryContact : (custName ? `contact@${custName.toLowerCase().replace(/[^a-z0-9]/g, "")}.com` : "contact@client.com")),
